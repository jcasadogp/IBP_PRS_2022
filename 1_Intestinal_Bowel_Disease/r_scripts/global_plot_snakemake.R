...
# * DONE: starting positions plink and prsice
# * DONE: target_prs.lasso <- read.table('IBD_GSA_fin_prs_lasso1.txt', header = T)[,1:3] ==> variable file name
...
# GET THEM FROM SNAKEMAKE
range_list <- "/staging/leuven/stg_00092/IBP_PRSproject/1_Intestinal_Bowel_Disease/output_data/a_1_julia_plink/target_data/range_list"
phenotype_file <- "/staging/leuven/stg_00092/IBP_PRSproject/1_Intestinal_Bowel_Disease/data/final_phenotypes.txt"
...
target_prs.prsice_file <- read.table(paste("a_2_julia_prsice/target_data/",target_prefix,".all_score",sep=""), header = T)
...
# Get EXTERNAL scores
external_prs.prsice <- read.table(paste("a_2_julia_prsice/external_data/",external_prefix,".all_score",sep=""), header = T)
